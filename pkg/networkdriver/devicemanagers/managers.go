...
func InitManagers(logger *slog.Logger, managerConfigs *v2alpha1.CiliumNetworkDriverDeviceManagerConfig) (map[types.DeviceManagerType]types.DeviceManager, error) {
	...
	if managerConfigs.RXQueue != nil && managerConfigs.RXQueue.Enabled {
		rxQueueMgr, err := rxqueue.NewManager(logger, managerConfigs.RXQueue)
		if err != nil {
			return nil, err
		}

		result[types.DeviceManagerTypeRXQueue] = rxQueueMgr
	}
