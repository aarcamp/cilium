...
func (d *RXQueueDevice) Setup(allocation types.DeviceAllocation) (types.Device, error) {
	...
	if prepared, exists, err := d.adoptExisting(hostIfName, peerIfName); err != nil {
		return nil, err
	} else if exists {
		return prepared, nil
	}
	...
	return d.createLease(physical, hostIfName, peerIfName)
}
...
func (d *RXQueueDevice) adoptExisting(hostIfName, peerIfName string) (*RXQueueDevice, bool, error) {
